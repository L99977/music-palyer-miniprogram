...
Page({
   ...
   */
  data: {
    videoGroupList: [], // 获取导航标签的内容
    navId: '', // 当前显示标签页的id
    videoList: [], // 视频列表数据
    videoId: '', //视频的id标识
    videoUpdateTime: [], //用于存放video播放的时长的数组
    istriggered: false, // 用于标识下拉刷新是否被触发
  ...
  async getVideoGroupListData() {
    ...
    this.setData({
      ...
      navId: videoGroupListData.data[0].id // 第一个显示的id
    })
    // 获取视频数据
  ...
  async getVideoList(navId) {
    ...
    console.log("视频数据", VideoListData);
  ...
  handlePlay(event) {
    ...
    * */
    ...
    // 判断当前的视频是否播放过，是否有播放记录，如果有播放记录，就跳转至相应的记录位置
  ...
  handleEnded(event) {
    console.log('已结束');
    // 移除记录播放时长数组videoUpdateTime中当前结束了的视频
    let { videoUpdateTime } = this.data;
    // 找到在数组中的下标
  ...
  // 自定义上拉触底的回调函数 ---> scroll-view
  handleToLower() {
    console.log('上拉触底 || 网易没有提供上拉触底API');
    // 添加视频到视频列表videoList中
    //模拟数据
     let newVideoList = [{
         "type": 1,
  ...
  onShareAppMessage: function ({from}) {
    console.log(from); // button / menu
    if (from === 'button') {
      return {
        title: '来自button的转发',
        path: "/pages/video/video",
        imageUrl: '/static/images/George.jpg'
      }
    } else {
      return {
        title: '来自menu的转发',
